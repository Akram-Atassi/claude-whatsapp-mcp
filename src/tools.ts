...
export const SERVER_INFO = { name: "whatsapp", version: "1.0.0" } as const;
export const SERVER_INSTRUCTIONS =
  "Read and send WhatsApp messages from the user's own linked account. Always confirm the recipient with the user before sending anything.";
...
/** How long get_login_qr waits for a QR to appear before giving up. */
export const QR_WAIT_MS = 40_000;

/**
 * Registers all tools on `server`. Every tool declares all four MCP
 * annotation hints (readOnlyHint, destructiveHint, idempotentHint,
 * openWorldHint) as explicit booleans so hosts can warn before invoking.
 *
 * Kept separate from the stdio bootstrap in index.ts so tests can register the
 * same tools on an in-memory server.
 */
  ...
  server.registerTool(
    ...
    async () => {
      try {
        ...
        for (let i = 0; i < QR_WAIT_MS / 1000 && !whatsapp.lastQr; i++) await new Promise((r) => setTimeout(r, 1000));
        const qr = whatsapp.lastQr;
        if (!qr) return text(`No QR available (state: ${whatsapp.state}). Try again in a few seconds.`);

        const ascii = await QRCode.toString(qr, { type: "utf8" });
